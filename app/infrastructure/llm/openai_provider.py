...
class OpenAIProvider(LLMProvider):
    ...
    async def generate(self, system_prompt: str, user_prompt: str) -> LLMResponse:
        try:
            ...
            raise LLMProviderError("OpenAI rate limit exceeded", self.provider_name, e) from e
        except APIConnectionError as e:
            raise LLMProviderError("Cannot connect to OpenAI API", self.provider_name, e) from e
        except APIError as e:
            raise LLMProviderError(f"OpenAI API error: {e.message}", self.provider_name, e) from e
